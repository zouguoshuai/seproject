...
import requests
from fastapi import Cookie, Depends, FastAPI, HTTPException, responses
...
from schemas import Comment as c
...
def get_comment(order_id: int, db: Session = Depends(get_db)):
    ...
    comment = c(user_score=db_comment.user_score, user_text=db_comment.user_text, order_id=db_comment.order_id,
                store_text=db_comment.store_text,
                store_time=db_comment.store_time,
                user_time=db_comment.user_time)
    if not db_comment:
        raise HTTPException(status_code=400, detail="Comment not found")

    return schemas.CommentDict(msg="succeed", data=comment)
